...
public class MainTest {
    ...
    public void testSoapService() throws Exception {
        HttpURLConnection conn = getURLConnection(serverUrl + "/metro/SoapWsService", "POST");
    ...
    public void testFromWSDL() throws Exception {
        HttpURLConnection conn = getURLConnection(serverUrl + "/metro/addnumbers?wsdl", "GET");
        conn.setRequestProperty("Accept", "text/xml; charset=UTF-8");
        conn.setRequestProperty("Content-Type", "text/xml; charset=UTF-8");
        Assertions.assertEquals(200, conn.getResponseCode(), "SOAP response");
        ...
        AddNumbersService client = new AddNumbersService(new URL(serverUrl + "/metro/addnumbers?wsdl"));
